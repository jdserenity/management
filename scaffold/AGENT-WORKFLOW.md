...
- Only ever use surgical edits. Before every edit, re-read the current file. Use the smallest possible patch. If the expected old text no longer matches exactly, stop and re-read rather than rewriting the file. If a file has changed since you last touched, it is almost always me changing things around, and I do not want you to overwrite my changes.
