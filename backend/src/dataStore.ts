  ...
  getData(userId: string): UserData {
    ...
    return {
      ...
      streakActivities: this.db.prepare<[string], StreakActivity>(
        'SELECT id,name,description,frequency,weekly_target,scheduled_days_json,can_fail,necessary,archived_at,sort_order,linked_staple_id,linked_water,extra_calories,extra_protein,extra_water_ml,updated_at FROM streak_activities WHERE user_id=? ORDER BY sort_order'
      ).all(uid),
  ...
  putData(userId: string, data: UserData): void {
    ...
    this.db.transaction(() => {
      ...
      for (const row of data.streakActivities) {
        this.db.prepare(`
          INSERT INTO streak_activities (id,user_id,name,description,frequency,weekly_target,scheduled_days_json,can_fail,necessary,archived_at,sort_order,linked_staple_id,linked_water,extra_calories,extra_protein,extra_water_ml,updated_at)
          VALUES (?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?)
        `).run(row.id, uid, row.name, row.description ?? null, row.frequency, row.weekly_target ?? null, row.scheduled_days_json ?? null, row.can_fail, row.necessary ?? 0, row.archived_at ?? null, row.sort_order, row.linked_staple_id ?? null, row.linked_water ?? 0, row.extra_calories ?? null, row.extra_protein ?? null, row.extra_water_ml ?? null, row.updated_at);
      }
