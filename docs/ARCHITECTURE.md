...
- **Daily tab** (`DailyPage.tsx`) is the default landing tab: **morning stretch** (custom wake-up routine, top section), then a stacked **tracker deck** (`TrackerDeck.tsx`) with three overlapping panels always visible — **Streaks**, **TDEE**, and **Water**. One panel is in front (full opacity); the other two stay visible but faded behind. Default front: Streaks (TDEE left, Water right). Click a faded side or swipe to bring it forward (no tab pills). Streak activities can optionally cross-log calories, protein, and water on success. Data in `src/lib/tdeeDb.ts`, `src/lib/waterDb.ts`, `src/lib/streakDb.ts`, `src/lib/morningStretch/morningStretchDb.ts`; day boundary `stats_day_rollover_hour_v1`.
