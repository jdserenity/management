...
- Remote sync goes through `apps/server` (`npm run dev:server`, default `http://localhost:8787`). Clients authenticate with `SERVER_TOKEN` / `VITE_SERVER_TOKEN` (see `.env.example`). Both desktop and companion read `VITE_SERVER_URL` and `VITE_SERVER_TOKEN` from the root `.env` at **build time** (Vite bakes them into the bundle via `getBuildTimeSyncCreds` in `@mgmt/sync`); there is no separate runtime sync config file.
