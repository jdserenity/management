...
## Mobile companion
- PWA in `apps/companion` (`npm run dev:companion`, port 5173). Production build lazy-loads tab pages and opens local sql.js storage before background server pull so first paint is not blocked on the network. Service worker precaches built assets (including sql-wasm) for reliable offline installs.
