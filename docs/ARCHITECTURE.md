...
- Settings toggle **Can't exercise right now** (`cant_exercise_mode_v1` in `app_kv`, `src/lib/cantExerciseModePref.ts`) limits scheduled break workouts to walking/marching and stretches only (no push-ups, jacks, squats, or shadowboxing); applies globally, not per flow.
