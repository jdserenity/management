...
- `shared/sync-client/`: sync client interface, HTTP client, sync status helpers, in-memory bus (`@mgmt/sync`).
- `backend/server/`: SQLite-backed HTTP server for active session sync and shared user data (`@mgmt/server`).
...
- Env vars: `SERVER_TOKEN` (bearer auth), `PORT` (default 8787), `DB_PATH` (optional absolute path; defaults to `backend/server/data/server.db`).
