...
- **Daily tab** (`DailyPage.tsx`) is the default landing tab: **scheduled stretches** (built-in morning stretch plus user-created routines with a Daily-tab trigger, top section via `DailyStretchSections.tsx`), then **Streaks** (`StreakSection`), **TDEE** (`TdeeSection`), and **Water** (`WaterSection`). Streak success can optionally cross-log calories, protein, and water when configured on an activity. Data: `src/lib/stretchCreator/stretchCreatorDb.ts`, `src/lib/streakDb.ts`, `src/lib/tdeeDb.ts`, `src/lib/waterDb.ts`. Day boundary uses `stats_day_rollover_hour_v1` (same as work/movement stats).
...
## Mobile companion
- PWA in `apps/companion` (`npm run dev:companion`, port 5173). `CompanionBoot.tsx` mounts immediately with a loading screen while sql.js storage, i18n, and the app shell load in parallel; tab pages stay lazy-loaded. Service worker precaches built assets (including sql-wasm) for reliable offline installs.
...
- Companion storage: sql.js SQLite backed by IndexedDB (`mgmt-companion-sql`). On startup, `runCompanionInitialSync` in `CompanionBoot` pulls `GET /v1/data`, merges with local data when both sides have rows (per-row last-write-wins via `mergeUserData` in `@mgmt/sync`), then may push `POST /v1/data` when the server was empty or after a merge. Foreground returns trigger another pull/merge. Synced tables include focus/workout logs, `app_kv`, nutrition, streaks (including cross-log columns), and water. Outbound debounced pushes wait for initial sync bootstrap to finish and never send an empty snapshot. Failures log to the browser console with URL, HTTP status, and response body; `CompanionSyncBanner` shows a warning when initial sync fails.
...
- Desktop sync server URL and token load at **runtime** from `sync-server.json` (Tauri plugin store in the app config dir, e.g. macOS `~/Library/Application Support/com.diamari.management/sync-server.json`) via `src/lib/syncServerConfig.ts`. On first launch, if that file is empty and `VITE_SERVER_URL` / `VITE_SERVER_TOKEN` are set (local dev or after rebuild), values are copied into the store once. If the store already has values but build-time `VITE_*` env vars differ (e.g. after switching from a Tailscale IP to `https://mgmt.levier.cc`), the store is updated to match the build env. Companion/Pages still use build-time `VITE_*` env vars. On startup, desktop runs `runDesktopInitialSync` (`src/lib/dataSync.ts`): pull from server, merge when both sides have data (`mergeUserData`), push when server was empty or after merge; foreground focus triggers another pull/merge. Debounced pushes after local writes wait until initial sync finishes (`src/lib/dataSyncBootstrap.ts`). UI listens for `DATA_SYNC_REFRESH_EVENT` from `@mgmt/sync` to reload Daily/Customize sections after a pull.
...
- `server.db` (`apps/server/data/server.db` by default, overridden with `DB_PATH`) mirrors all `local.db` tables plus a `users` table and `user_id` on every row. Owner user ID is seeded on server startup from `OWNER_USER_ID` env var (default `'owner'`). Companion reads data via `GET /v1/data`; full data can be pushed via `POST /v1/data`. **Sync safety:** clients never `POST` a fully empty snapshot; `hydrateDbFromServer` skips applying an empty server pull when local data exists; the server rejects `POST /v1/data` with HTTP 409 when the payload would replace existing rows with an empty snapshot (`DataWipeRefusedError` in `@mgmt/sync`).
  - Tables in `local.db`:
    ...
    - `water_config`, `water_entries` — daily water target (ml) and today-only log entries (`src/lib/waterDb.ts`, migration v8).
    - `streak_activities`, `streak_log_cells`, `streak_activity_meta` — habit definitions (including optional `extra_calories`, `extra_protein`, `extra_water_ml` for cross-log on success), per-day log cells, pause/reset meta (`src/lib/streakDb.ts`, migrations v6 and v9).
