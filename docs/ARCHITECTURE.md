...
- **Daily tab** (`DailyPage.tsx`) is the default landing tab: **morning stretch** (top), then **Streaks** (`StreakSection`), **TDEE** (`TdeeSection`), and **Water** (`WaterSection`). Streak success can optionally cross-log calories, protein, and water when configured on an activity. Data: `src/lib/streakDb.ts`, `src/lib/tdeeDb.ts`, `src/lib/waterDb.ts`, `src/lib/morningStretch/morningStretchDb.ts`. Day boundary uses `stats_day_rollover_hour_v1` (same as work/movement stats).
