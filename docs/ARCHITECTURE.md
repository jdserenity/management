...
- Companion storage: sql.js SQLite backed by IndexedDB (`mgmt-companion-sql`). On startup and whenever the PWA returns to the foreground (`visibilitychange` / `focus`), fetches `GET /v1/data` and replaces local synced tables (`hydrateDb` in `@mgmt/sync`). Synced tables include focus/workout logs, `app_kv`, nutrition, streaks (including cross-log columns), and water. Outbound pushes wait for the first pull to finish so an empty local DB cannot overwrite server data. Habit UIs listen for `mgmt-companion-data-refresh` to reload. After each local write, a debounced push via `POST /v1/data` keeps the server current (`pushUserData` / `extractUserData` in `@mgmt/sync`). `POST /v1/data` and `hydrateDb` treat the payload as a full snapshot (rows missing from the payload are removed). If `VITE_SERVER_URL` is unset, sync is silently skipped.
