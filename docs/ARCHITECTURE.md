...
- **Morning stretch** (`MorningStretchSection.tsx` on Daily tab): user-defined ordered routine stored in `app_kv` as `morning_stretch_routine_v1` (`src/lib/morningStretch/`). Exercise choices come only from the global pool enabled in Customize workouts (`workout_customize_prefs_v1` — predefined moves, stretch picks, custom exercises). Completion logs to `workout_log` with `workout_id` `morning-stretch` and counts toward movement stats. “Done today” uses the same stats-day rollover as work and nutrition; user can still run again via **Do again**.
