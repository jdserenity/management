...
- Active focus flows end when the machine sleeps, the display sleeps, or the screen locks (macOS emits `flow-suspend`; all platforms also detect a timer heartbeat gap after resume). Partial focus credit applies if the focus phase ran ≥15 seconds.
...
| `flow-suspend` | macOS system sleep, display sleep, or screen lock | `SessionContext.tsx` |
