...
- Desktop sync uses the same build-time `VITE_*` creds (from local `.env` at `tauri build`) and the same `runBidirectionalInitialSync` / `pullAndMergeUserData` helpers as the companion. `DesktopBoot` (`src/components/DesktopBoot.tsx`) shows `SyncBootScreen` until local SQLite is open and initial sync completes, then mounts `App`. Debounced pushes after local writes wait until initial sync finishes (`src/lib/dataSyncBootstrap.ts`). UI listens for `DATA_SYNC_REFRESH_EVENT` to reload Daily/Customize sections after a pull.
