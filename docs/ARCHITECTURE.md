...
- **Customize tab** (`CustomizePage.tsx`) has four subtabs — **Exercises**, **Stretches**, **Habits**, **Food** — and sits between Stats and Settings in the nav. **Stretches** (`CustomizeStretchesPanel.tsx`): stretch creator — name, emoji, colour gradient, ordered exercise refs (same pool as break workouts), block duration, enabled toggle, trigger **scheduled** (shows on Daily until hide-after hour or completion) or **manual** (not surfaced on Daily yet). Built-in **morning stretch** ships with the app and cannot be deleted. Habits: add/edit/pause/archive/reset streak activities. Food: TDEE/protein targets, staples, and regulars edited as ingredients (totals sum from ingredients) or as simple no-ingredient items. Daily tab is log-only (check off habits, log food).
