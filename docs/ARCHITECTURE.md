...
- **Daily tab** (`DailyPage.tsx`) is the default landing tab: **morning stretch** (custom wake-up routine, top section), then a stacked **tracker deck** (`TrackerDeck.tsx`) with three overlapping panels — **habits** (streak tracker heatmaps, daily/weekly activities), **calories** (TDEE targets, today’s food log, staples/regulars), and **water** (daily ml target, quick-add amounts, today’s log). One panel is fully visible; the other two peek from left/right (click or swipe to bring forward). Tab pills above the deck also switch panels. Habit activities can optionally cross-log calories, protein, and water on success (badges on the row; one click logs all). Ported from Obsidian plugins; data lives in SQLite (`src/lib/tdeeDb.ts`, `src/lib/waterDb.ts`, `src/lib/streakDb.ts`, `src/lib/morningStretch/morningStretchDb.ts`). Day boundary uses the same `stats_day_rollover_hour_v1` as work/movement stats.
...
flowchart TB
  ...
  subgraph tabs["Active tab component src/components/"]
    Daily["DailyPage.tsx morning stretch TrackerDeck habits calories water"]
