...
- Companion storage: sql.js SQLite backed by IndexedDB (`mgmt-companion-sql`). On startup, `runCompanionInitialSync` in `CompanionBoot` pulls `GET /v1/data` then pushes `POST /v1/data` (same peer pattern as desktop). Foreground returns trigger another pull. Synced tables include focus/workout logs, `app_kv`, nutrition, streaks (including cross-log columns), and water. Outbound debounced pushes wait for initial sync to finish. Failures log to the browser console with URL, HTTP status, and response body; `CompanionSyncBanner` shows a warning when initial sync fails.
