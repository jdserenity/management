...
- **Daily tab** (`DailyPage.tsx`) is the default landing tab: **morning stretch** (custom wake-up routine, top section), then a stacked **tracker deck** (`TrackerDeck.tsx`) with three overlapping panels — **Streaks** (streak tracker heatmaps, daily/weekly activities), **TDEE** (targets, today’s food log, staples/regulars), and **Water** (daily ml target, quick-add amounts, today’s log). Default: Streaks centered; TDEE peeks from the left and Water from the right (~half visible, faded). Click a peeking side or swipe to bring that panel forward (no tab pills). Streak activities can optionally cross-log calories, protein, and water on success (badges on the row; one click logs all). Data lives in SQLite (`src/lib/tdeeDb.ts`, `src/lib/waterDb.ts`, `src/lib/streakDb.ts`, `src/lib/morningStretch/morningStretchDb.ts`). Day boundary uses `stats_day_rollover_hour_v1`.
...
- **Morning stretch** (`MorningStretchSection.tsx` on Daily tab): user-defined ordered routine stored in `app_kv` as `morning_stretch_routine_v1` (`src/lib/morningStretch/`). Default routine: neck roll, lateral shoulder stretch, hip roll, arm rolls, deep squat, standing forward hang. Exercise choices come only from the global pool enabled in Customize workouts (`workout_customize_prefs_v1` — predefined moves including arm rolls, reverse lunges, reverse crunches, plank; stretch picks including standing forward hang; custom exercises). Timed block (default 5 min, `morning_stretch_duration_minutes_v1`); completion logs to `workout_log` with `workout_id` `morning-stretch` at proportional volume and counts toward movement stats. Section hides completely after completion or after the hide-after time (default 11:00 AM local, `morning_stretch_hide_after_hour_v1`) until the next stats day; toggle and timing prefs in Settings (`morning_stretch_enabled_v1`, default on).
...
flowchart TB
  ...
  subgraph tabs["Active tab component src/components/"]
    Daily["DailyPage.tsx morning stretch TrackerDeck Streaks TDEE Water"]
