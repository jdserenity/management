...
- Companion storage: sql.js SQLite backed by IndexedDB (`mgmt-companion-sql`). On startup, fetches a full data snapshot from `GET /v1/data` and replaces local synced tables (`hydrateDb` in `@mgmt/sync`). After each local write, a debounced push via `POST /v1/data` keeps the server current (`pushUserData` / `extractUserData` in `@mgmt/sync`). `POST /v1/data` and `hydrateDb` treat the payload as a full snapshot (rows missing from the payload are removed). If `VITE_SERVER_URL` is unset, sync is silently skipped.
