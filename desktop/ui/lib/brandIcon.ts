...
/** Solid white rounded square for the macOS menu bar tray (always white, full opacity). */
...
/** Same solid white square when monitoring is off — never a faded / translucent icon. */
export function brandTrayMonitoringOffSvg(size = 1024, fill = '#FFFFFF', insetRatio = 0.0625): string {
  return brandTrayIconSvg(size, fill, insetRatio);
}
