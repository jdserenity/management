...
import { getCurrentDay, parseDate, formatDate, daysBetween, getISOWeekStart, getWeekDays } from '@/lib/streak/dates';
...
export const calculateStats = (
  data: StreakData,
  activityId: string,
  activityConfigMap: Record<string, StreakActivity> | undefined,
  dayEndTime: string
): void => {
  const activity = activityConfigMap?.[activityId];
  if (activity?.frequency === 'weekly') {
    calculateWeeklyStats(data, activityId, activity.weeklyTarget || 1, dayEndTime);
    return;
  }
  ...
  if (!startDate) {
    data.stats[activityId] = { currentStreak: 0, longestStreak: 0, totalSuccesses: 0, totalDays: 0 };
    return;
  }
  ...
  if (getLogState(todayLog?.[activityId]) !== 'success') checkDate.setDate(checkDate.getDate() - 1);
  ...
  data.stats[activityId] = { currentStreak, longestStreak, totalSuccesses, totalDays };
};

export const recalculateAllStats = (
  data: StreakData,
  ...
  for (const dateStr of Object.keys(data.logs)) {
    for (const activityId of Object.keys(data.logs[dateStr])) activityIds.add(activityId);
  }
  for (const act of Object.keys(data.activityStartDates)) activityIds.add(act);
  for (const activityId of activityIds) calculateStats(data, activityId, activityConfigMap, dayEndTime);
  return activityIds;
};
